 ...
 */
function save_and_break_edge($handles, $edgePoints, $vertexIndex, $idEdge, &$errorMsg, &$breakedPolylines, $replaceOld = true) {
	$mysqli = $handles[0];
	$nodeModel = $handles[1];
	$edgeModel = $handles[2];
	...
	//-- Get edge info
	$edgeData = $edgeModel->get_edge_by_id($idEdge);
	...
	if ($newIdNode) {
		...
		if ($replaceOld) {
			$newIdEdge = $idEdge;
			$edgeData1['edge_name'] = _db_to_query($edgeData['edge_name'], $mysqli);
		} else {
			$edgeData1['edge_name'] = _db_to_query($edgeData['edge_name']." #2", $mysqli);
		}
